...
import { PrototypeProductDriver } from '../drivers/prototype/driver.js';
import { Prototypes, PrototypeError, prototypeIdSchema, prototypeManifestSchema, prototypeSummary } from '../prototypes/store.js';
...
export function createServer(config: AppConfig) {
  const server = new McpServer({ name: 'usability-test-mcp', version: '0.10.0' }, {
  ...
  const prototypes = new Prototypes(config.artifactRoot);
  const orchestrator = new SessionOrchestrator(recorder, input => {
    if (input.platform === 'native') return new MaestroProductDriver();
    if (input.platform === 'prototype') return new PrototypeProductDriver(prototypes);
  ...
  const prototypeResponse = async (operation: () => Promise<object>) => {
    try { return result(await operation()); }
    catch (error) { return result({ error: error instanceof PrototypeError ? error.message : error instanceof z.ZodError ? `Invalid prototype manifest: ${error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('; ').slice(0, 600)}` : 'Prototype import failed. Check image paths or URLs and the manifest fields.' }, true); }
  };
  server.registerTool('usability_import_prototype', {
    description: 'Setup coordinator only: import a static design flow (e.g. Figma frames in a section, in left-to-right order) as PNG screens plus optional tap targets. Images may be absolute file paths, https/localhost URLs or base64. Targets are evaluator-only and never shown to participants. The importing chat has now seen the flow: run participants in a fresh chat or agent.',
    inputSchema: prototypeManifestSchema,
  }, async manifest => prototypeResponse(async () => ({ prototype: prototypeSummary(await prototypes.import(manifest)), nextTool: 'usability_test_prototype',
    instructions: 'Show the owner the screen order and which steps are scored (owner-marked or proposed-approved targets) or unscored. Then start participants with usability_test_prototype from a FRESH context (contextCheck first-visit, host-reported fresh), or save a project plan with platform=prototype and target=prototypeId.' })));
  server.registerTool('usability_test_prototype', {
    description: 'Start a usability test on an imported static design flow. The participant sees one frame at a time and taps with tap_point; marked targets advance, other taps show nothing (3 misses and the facilitator moves on), unmarked steps always advance. contextCheck is required. Follow nextTool until finished.',
    inputSchema: z.strictObject({ prototypeId: prototypeIdSchema, goal: z.string().min(1).max(2000), scenario: z.string().min(1).max(4000).optional(), audience: z.string().min(1).max(2000).default('A first-time user'),
      contextCheck: contextCheckSchema, rerun: rerunSchema.optional(), exercise: z.enum(['task', 'first-impression']).default('task'), participantCount: z.number().int().min(1).max(5).default(1), maxActions: z.number().int().min(1).max(100).default(25) })
      .refine(x => x.exercise === 'task' || x.participantCount === 1, 'First-impression exercises run one participant and are never pooled'),
  }, async ({ prototypeId, goal, scenario, audience, contextCheck, rerun, exercise, participantCount, maxActions }, ctx) => respond(async () => {
    const prototype = await prototypes.get(prototypeId).catch((error: Error) => { throw new HostSessionError(error.message); });
    return host.start(participantCount === 1 ? 'session' : 'round', { platform: 'prototype', target: prototypeId, goal, contextCheck, rerun, ...(exercise === 'first-impression' ? { exercise } : {}),
      scenario: scenario ?? (exercise === 'first-impression' ? 'You have just opened this design for the first time.' : 'You are using this product for the first time to pursue the supplied goal.'),
      ...(participantCount === 1 ? { persona: { context: audience } } : { participantCount, personaContext: audience }),
      viewport: prototype.device, maxActions, timeoutMs: 600000, accessibilityChecks: false }, ctx.mcpReq.signal);
  }));
  server.registerTool('usability_health', { description: 'Check local server status. Reasoning is supplied by the connected chat; no model API credentials.', inputSchema: z.strictObject({}) },
    async () => result({ status: 'ok', methodologyVersion: METHODOLOGY_VERSION, platforms: ['web', 'mobile-web', 'figma-prototype', 'native-experimental'], reasoningMode: 'connected-host-chat', apiKeyRequired: false, artifactRoot: config.artifactRoot }));
  ...
  server.registerPrompt('test-figma-flow', {
    description: 'Usability-test a static Figma flow (frames laid out left to right in a section) with the same method as websites.',
    argsSchema: z.object({ figmaUrl: z.string(), goal: z.string().optional() }),
  }, ({ figmaUrl, goal }) => ({ messages: [{ role: 'user' as const, content: { type: 'text' as const,
    text: `Set up a usability test for the Figma flow at ${figmaUrl}.${goal ? ` Goal: ${goal}.` : ''} Use whatever Figma MCP or CLI you have. 1) Find the section (or frames) for this flow and list its top-level frames sorted left to right, then top to bottom. Confirm the order with the owner. 2) Export each frame as a PNG at 1x to an absolute file path (or give a https/localhost asset URL, or base64). With the plugin API: node.exportAsync({ format: 'PNG', constraint: { type: 'SCALE', value: 1 } }). 3) For each frame collect only VISIBLE text (skip hidden layers) as visibleText. 4) Tap targets: if the designer marked the element that leads to the next frame (for example a layer named starting with @tap, or an annotation), use its box relative to the frame (x/y from frame top-left, in frame pixels) with basis owner-marked. Otherwise you may propose one target per step from the flow and use case, show them to the owner, and only keep approved ones as proposed-approved. Steps without a target are unscored: the evaluator judges whether the tap made sense. 5) Call usability_import_prototype with the manifest. 6) Ask the owner the usual setup questions (audience, the task in the user's words, what success looks like). Never put frame names, screen counts or target positions in the task wording. 7) This chat has now seen every frame, so run the participant in a FRESH chat or agent with only: prototypeId, the neutral goal, audience and contextCheck {mode:"first-visit",isolation:"host-reported fresh",exposure:[]}, via usability_test_prototype. 8) After the journey, submit evidence-linked findings (misclicks and moved-on steps are recorded obstacles) and read the executive report. Static designs have no hover, loading, typing or scrolling; say so rather than inventing interaction issues.` } }] }));
