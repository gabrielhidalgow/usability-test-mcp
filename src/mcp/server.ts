...
import { contextCheckSchema, rerunSchema, discoveryIdSchema, handoffSchema, roundInputSchema, sessionInputSchema, type AppConfig } from '../config/schema.js';
...
export function createServer(config: AppConfig) {
  const server = new McpServer({ name: 'usability-test-mcp', version: '0.7.1' }, {
  ...
  server.registerTool('usability_quick_test', {
    ...
    inputSchema: z.strictObject({ target: z.url().refine(value => { try { const u = new URL(value); return ['http:', 'https:'].includes(u.protocol) && !u.username && !u.password; } catch { return false; } }), goal: z.string().min(1).max(2000), audience: z.string().min(1).max(2000).default('A first-time visitor'), device: z.enum(['desktop', 'mobile']).default('desktop'), presentation: z.enum(['visible', 'background']).optional(), handoff: handoffSchema.optional(), contextCheck: contextCheckSchema.optional(), rerun: rerunSchema.optional(), participantCount: z.number().int().min(1).max(5).default(1), accessibilityChecks: z.boolean().default(false) }),
  }, async ({ target, goal, audience, device, presentation, handoff, contextCheck, rerun, participantCount, accessibilityChecks }, ctx) => respond(() => host.start(participantCount === 1 ? 'session' : 'round', { target, goal, presentation, handoff, contextCheck, rerun, scenario: 'You are using this product for the first time to pursue the supplied goal.', ...(participantCount === 1 ? { persona: { context: audience } } : { participantCount, personaContext: audience }), viewport: device, maxActions: 12, timeoutMs: 600000, accessibilityChecks }, ctx.mcpReq.signal)));
