...
- **Paths:** `/pricing` also covers `/pricing/...`. A path of `/` means **the home page only** (leave the focus out entirely to test the whole site).
...
This classification uses timing and destination, not reliable causal attribution. After the first browser interaction, blocked writes to **third-party sites** (analytics, ad pixels) still only reduce fidelity; they are aborted, never sent. Blocked writes to the **product's own site**, including its subdomains (for example `api.example.com.au` for `www.example.com.au`), remain fatal after an interaction, including delayed handlers and requests on later pages, because they may be the participant's own submission. Diagnostics label each destination `same-origin`, `same-site` or `third-party`; the site match is an approximation of the registrable domain. Reports group blocked requests by effect: **third-party tracking** (aborted writes to other sites; no effect on what was shown), **page resources** (scripts, images, frames; may change what was shown), **site writes** and **navigations**. Only the last three count as a coverage gap or trigger the participant's reduced-fidelity warning; the appendix collapses repeated events into one row with a count. Main-frame navigation blocks and non-background mutations remain fatal. A missing interface cannot be claimed successful merely because the run continues. Inspect visible evidence and do not label harness-altered content a product defect. No communication capability or test-environment override is needed for this change.
