...
Keep interpretations and recommendations separate from observable behavior. Do not invent issues, human feelings, or visual conclusions unsupported by screenshots. Harness safety blocks, time limits, and infrastructure errors are not product usability issues. Inspect policyDiagnostics and warnings: missing or altered content may be caused by blocked background resources, so do not attribute it to the product without independent evidence. No score or statistical claims. ${CHANGE_INSTRUCTIONS}`;
